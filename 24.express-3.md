...
- get 方法用于往路由里添加一条路由规则
- 初始化 router 对象保存所有的路由
- listen 方法用于启动一个 HTTP 服务器并指定处理函数
...
```
[构建基本服务器](https://gitee.com/zhufengnodejs/zf-express/commit/6d1ddf74c81c59406141fa234caf5ee60d132366)
...
Layer.ptototype.match = function(path){
...
methods.forEach(function(method){
    Router.prototype[method] = function(path) {
        // 创建路由实例，添加 Router Layer
        let route = this.route(path);
        // 调用路由方法 添加 route Layer
        route[method].apply(route, slice.call(arguments, 1));
        return this;
    }
});
...
- 可以获取 req.params
- 提供 app.param 的能力
    - layer 借助 path-to-regexp 提取 params
    - 在 Router.handle里，process_params函数一次调用参数处理函数
## 4.1 测试用例
```js
const express = require('../');
const app = express();
