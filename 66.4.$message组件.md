...
# 编写Message组件
要考虑数据驱动视图，多次点击显示多个弹层，给每个层增加id号，延迟时间和内容
```js
<template>
    <div class="messages">
        <!-- 显示在数组中的层 -->
    ...
    export default {
        ...
        methods: {
            add(option) {
                let id = this.id++; // id号
                let layer = {...option, id};
                this.message.push(layer); // 将层存储起来
            ...
            remove(layer) {
                ...
                // 移除层
                this.message = this.messages.filter(message => layer.id !== message.id);
            }
        }
    }
</script>
