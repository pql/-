...
```
## 在组件中获取数据
对 element-ui 树组件进行二次封装，封装TreeComponent组件
```js
<template>
    <div>
...
render(h, {node, data, store}) {
    ...
    return (
        <div>
            {parent?
                node.expanded?
                <i class="el-icon-folder-opened icon"> : <i class="el-icon-folder icon"></i>:
...
```js
<TreeComponent :data="treeList"
            :rootList="rootList"
            :childList="childList"></TreeComponent>
rootList: [
    ...
    this.currentId === data.id?
        <span>
            <el-input 
                ...
                on-input={this.handleChange}></el-input>
            <el-button type="text">
...
```
## 调用接口删除文件
如果用户传递了delete方法，调用成功后再更新页面数据
```
```js
<TreeComponent :data.sync="treeList"
:rootList="rootList"
:childList="childList"
:delete="deleteFn">
