...
- 修饰器本质就是编译时执行的函数
- 如果想添加实例属性，可以通过目标类prototype对象操纵
```js
@testable
class Person {
...
# 8.使用对可观察对象做出响应
## 8.1 computed
- 计算值(computed values)是可以根据现有的状态或其他计算值衍生出的值
...
```
## 8.4 reaction
- autorun的变种，autorun会自动触发，reaction对于如何追踪 observable 赋予了更细粒度的控制
        ...
        return (
            <div className="todo-list">
                ...
                <p>
                    ...
                    className={filter==='uncompeted'?'active':''}>未完成</button>
...
class TodoFooter extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <span>你还有{left}件待办事项！</span>
                <button onClick={() => changeFilter('all')} className={filter==='all'?'active':''}>全部</button>
                <button onClick={() => changeFilter('uncompleted')} className={filter==='uncompleted'?'active':''}>未完成</button>
                <button onClick={() => changeFilter('completed')} className={filter==='completed'?'active':''}>已完成</button>
            </div>
        )
    }
}
