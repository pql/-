...
class Login extends Component {
    handleSubmit = (event) => {
        ...
        this.loginForm.props.form.validateFields((err, values) => {
            if(err) {
                return message.wran('登录信息输入不合法！请重新输入！');
            } else {
...
class Rooms extends Component {
    handleSubmit = (event) => {
        event.preventDefault();
        this.loginForm.props.from.validateFields((err, values) => {
            if(err) {
                return message.warn('登录信息输入不合法！请重新输入！');
            } else {
                this.props.dispatch({type: 'login/login', payload: values});
                                    ...
                                    <Col span={6} style={{padding: '5px 0'}}>
                                        <Link to={`/rooms/${room._id}`}>
                                            ...
                                            cover={<img alt="example" src="https://os.alipayobjects.com/rmsportal/QBnOOoLaAfKPirc.png" />}>
                                                <Card.Meta title={room.name}></Card.Meta>
                                            </Card>
                                        </Link>
                                    </Col>
                                ))
...
export default {
    ...
    effects: {
        *login({payload}, {call,put}) {
            const {data: token, error, code} = yield call(serice.login,paylaod);
            if(code == 0) {
