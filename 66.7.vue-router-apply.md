...
```js
{
    name: 'home',
    path: '/home', // 实现了路由的懒加载
    components: { // 当前路径 匹配多个router-view
...
// 不同方式的传递参数
<td>
    <router-link :to="{name: 'userDetail', query: {id: u.id}}">{{u.id}}</router-link>
</td>
<td>
    <router-link :to="{name: 'detail', params:{name: u.username}}">{{u.username}}</router-link>
</td>
