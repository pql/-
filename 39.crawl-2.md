            ...
            <ul class="list-group">
                <%tags.forEach(tag=>{%>
                    <li class="list-group-item text-center">
                        <a href="/?tagId=<%=tag.id%>"><img style="width:25px;height:25px;" src="<%=tag.image%>"/><%=tag.name%></a>
                    </li>
                <%})%>
            </ul>
        </div>
...
app.get('/subscribe', async function(req, res){
    ...
    let selectedTagIds = selectedTags.map(item => item['tag_id']);
    tags.forEach(item => {
...
```
## 5.2 fromCodePoint
String.fromCodePoint() 静态方法返回使用指定的代码点序列的字符串。
```js
let str = '&#x65F6';
